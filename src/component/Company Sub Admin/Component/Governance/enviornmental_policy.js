...
export default class EnvoirnmentalPolicy extends Component {
  constructor(props) {
    super(props);
    this.state = {
      isLoaded: true,
      environmentalPolicy: [],
      environmentalSubAdminPolicy: [],
      environmentalPolicyGlobalData: [],
      errorMessage: [],
      policyComment: [],
      feedback: [],
      comment: "",
  ...
  setOpen3(e) {
    ...
    fetch(
      config.API_URL + `getGovernanceComments/${policyid}/${commentor}?current_role=${localStorage.getItem("role")}`,
      requestOptions
    )
      .then((res) => res.json())
      .then(
  ...
  handleSubmit3 = (event) => {
    ...
    axios
      .post(
        config.API_URL + `sendGovernanceComments?current_role=${localStorage.getItem("role")}`,
        {
      ...
      .then((response) => {
        ...
        window.location.href = baseURL + "/Environmental_Policy";
      })
  ...
  serverRequest() {
    ...
    fetch(
      config.API_URL + `getGovernanceTopics/${this.state.uuid}?current_role=${localStorage.getItem("role")}`,
      requestOptions
    )
      .then((res) => res.json())
      .then(
        (data) => {
          this.setState({
            ...
            environmentalAccess: data.environmentalPolicy_Access,
          });
        },
        (error) => {
          this.setState({
  ...
  getSubAdminPolicyData(user_id) {
    ...
    fetch(config.API_URL + `getSubAdminPolicyData/${user_id}?current_role=${localStorage.getItem("role")}`, requestOptions)
      .then((res) => res.json())
      .then(
        (data) => {
          this.setState({
            isLoaded: false,
            environmentalSubAdminPolicy: data.environmentalPolicy,
            environmentalAccess2: data.environmentalPolicy_Access,
          });
        },
        (error) => {
          this.setState({
    ...
    fetch(config.API_URL + `getGovernanceGlobalTopics?current_role=${localStorage.getItem("role")}`, requestOptions)
      ...
      .then(
        (data) => {
          this.setState({
            isLoaded: false,
            environmentalPolicyGlobalData: data.environmentalPolicy,
            environmentalAccess1: data.environmentalPolicy_Access,
          });
        },
        (error) => {
          this.setState({
        ...
        {this.state.isLoaded === false && (
          <div className="main_wrapper">
            <div className="question_type_filter">
              <div>
                <NavLink to="/governance_policies">Governance Policies</NavLink>
              </div>
              <div>
                        ...
                        <div className="Introduction framwork_2">
                          <div className="col-md-12">
                              ...
                              {currentUser.data.role === "company_admin" && (
                                <div className="col-xxl-3 col-lg-5 col-md-12 col-12 form_x mb-3 bold">
                                  <AssignSubAdminComponent3
                                    tableName="governance_EnvironmentalPolicy"
                                    removeHeader={true}
                                  />
                                </div>
                              )}
                            ...
                            <table>
                              ...
                              {environmentalPolicyGlobalData.map(
                                (item, key) => (
                                  <tr key={key}>
                                    <td>
                              ...
                              {environmentalPolicy.map((item, key) => (
                              ...
                              {environmentalSubAdminPolicy.map((item, key) => (
                                <div className="Statement_one" key={key}>
                                  <div className="Statement_2">
                                    <p className="statement_p">
                                      {item.question}
