...
const ScopeBar = ({ consumption, notGenerated, maxConsumption }) => {
  // Calculate the width percentages for each section of the bar
  const totalWidth = 100; // 100% width of the bar
  const filledWidth = (consumption / (consumption + notGenerated)) * totalWidth;
  const notGeneratedWidth =
    (notGenerated / (consumption + notGenerated)) * totalWidth;
  ...
  return (
    <div className="water-withdrawn-container">
      <div className="water-withdrawn-header">Scope 1 and Scope 2 Emission</div>
      <div className="water-withdrawn-bar-labels">
        ...
        <span style={{ fontSize: "11px", fontWeight: "bold", fontSize: "12px" }}>
          {Math.round((consumption + notGenerated) / 5 / 10) * 10}
        </span>
        <span style={{ fontSize: "11px", fontWeight: "bold", fontSize: "12px" }}>
          {Math.round((((consumption + notGenerated) / 5) * 2) / 10) * 10}
        </span>
        <span style={{ fontSize: "11px", fontWeight: "bold", fontSize: "12px" }}>
          {Math.round((((consumption + notGenerated) / 5) * 3) / 10) * 10}
        </span>
        <span style={{ fontSize: "11px", fontWeight: "bold", fontSize: "12px" }}>
          {Math.round((((consumption + notGenerated) / 5) * 4) / 10) * 10}
        </span>
        <span style={{ fontSize: "11px", fontWeight: "bold", fontSize: "12px" }}>
          {Math.round((consumption + notGenerated) / 10) * 10}
        </span>
      </div>
      ...
      <div className="water-withdrawn-bars">
        ...
        >
          <OverlayTrigger
            placement="top"
            overlay={renderTooltip(`Scope 1 : ${consumption} tCO2e`)}
          >
            <div
        ...
        >
          <OverlayTrigger
            placement="top"
            overlay={renderTooltip(`Scope 2: ${notGenerated} tCO2e`)}
          >
            <div
        ...
        <div
          className="water-withdrawn-legends"
