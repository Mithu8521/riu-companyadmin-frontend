...
const EmissionEntryModal = ({
  ...
  const isFormValid = currentEntry.location && 
                    currentEntry.period &&                   
                    currentEntry.category &&
                    !duplicateEntry
  ...
  const getDisplayName = (type, id, options) => {
    ...
    if (type === 'location') {
      return item?.unitCode || `${item?.location?.area || ""}, ${item?.location?.city || ""}`.trim()
    }
    if (type === 'period') {
      const periodOption = timePeriodOptions?.find(option => option.value === id)
    ...
    return item?.fuel_type || item?.fuel || item?.transport || item?.engine || id
  }
    ...
    >
      ...
      <Modal.Body style={{ 
          ...
          >
            ...
            <div>
              ...
              <div style={{ fontSize: "0.85rem", color: "#78350f" }}>
                An entry with the same location, period, financial year, category, and fuel details already exists. 
                Please modify the details or update the existing entry.
              </div>
            </div>
          </div>
        )}
        ...
        <Row className="g-4 mb-4">
          <Col md={6}>
            <Form.Group>
              <Form.Label className="fw-semibold mb-2 d-flex align-items-center" style={{ color: "#1e293b", fontSize: "0.9rem" }}>
                <div className="w-2 h-2 bg-primary rounded-circle me-2" style={{ width: "6px", height: "6px" }}></div>
                Financial Year *
              </Form.Label>
          ...
          <Col md={6}>
            <Form.Group>
              <Form.Label className="fw-semibold mb-2 d-flex align-items-center" style={{ color: "#1e293b", fontSize: "0.9rem" }}>
                <div className="w-2 h-2 bg-success rounded-circle me-2" style={{ width: "6px", height: "6px" }}></div>
                Location *
              </Form.Label>
              <div className="position-relative">
                <Form.Select
          ...
          <Col md={6}>
            <Form.Group>
              <Form.Label className="fw-semibold mb-2 d-flex align-items-center" style={{ color: "#1e293b", fontSize: "0.9rem" }}>
                <div className="w-2 h-2 bg-warning rounded-circle me-2" style={{ width: "6px", height: "6px" }}></div>
                Category *
              </Form.Label>
              <div className="position-relative">
                <Form.Select
          ...
          {currentEntry.category === 'stationary' ? (
            <>
              <Col md={6}>
                <Form.Group>
                  <Form.Label className="fw-semibold mb-2 d-flex align-items-center" style={{ color: "#1e293b", fontSize: "0.9rem" }}>
                    <div className="w-2 h-2 bg-info rounded-circle me-2" style={{ width: "6px", height: "6px" }}></div>
                    Fuel Type *
                  </Form.Label>
                  <div className="position-relative">
                    <Form.Select
              ...
              <Col md={6}>
                <Form.Group>
                  <Form.Label className="fw-semibold mb-2 d-flex align-items-center" style={{ color: "#1e293b", fontSize: "0.9rem" }}>
                    <div className="w-2 h-2 rounded-circle me-2" style={{ 
                      width: "6px", 
                      height: "6px",
                      backgroundColor: currentEntry.fuelType ? "#8b5cf6" : "#cbd5e1"
                    }}></div>
                    Fuel *
                  </Form.Label>
                  <div className="position-relative">
                    <Form.Select
                      value={currentEntry.subFuelType}
                      onChange={(e) => {
                        updateEntry("subFuelType", e.target.value)
                        setFuelName(e.target.value)
                      }}
                      disabled={!currentEntry.fuelType}
                      className="border-2 py-2 ps-3 pe-4"
                      style={{
                        backgroundColor: currentEntry.fuelType ? "white" : "#f8fafc",
                        borderRadius: "8px",
                        fontSize: "0.9rem",
                        height: "44px",
                        cursor: !currentEntry.fuelType ? "not-allowed" : "pointer",
                        opacity: !currentEntry.fuelType ? 0.6 : 1
                      }}
                    >
                      <option value="">
                        {currentEntry.fuelType ? "Choose fuel..." : "Select fuel type first"}
                      </option>
                      {fuels?.map((subFuel) => (
            ...
            <>
              ...
              <Col md={4}>
                <Form.Group>
                  ...
                  <div className="position-relative">
                    ...
                    >
                      <option value="">
                        {currentEntry.fuelType ? "Choose transport type..." : "Select fuel first"}  
                      </option>
                      {transportType?.map((subFuel) => (
                        <option key={subFuel.id} value={subFuel.transport}>
                          {subFuel.transport}
                        </option>
                      ))}
                    </Form.Select>
              ...
              <Col md={4}>
                <Form.Group>
                  ...
                  <div className="position-relative">
                    <Form.Select
                      value={currentEntry.engineType}
                      onChange={(e) => {
                        updateEntry("engineType", e.target.value)
                    ...
                    >
                      <option value="">
                        {currentEntry.fuelType ? "Choose engine type..." : "Select fuel first"}  
                      </option>
                      { engineTypeOptions?.map((subFuel) => (
                        <option key={subFuel.id} value={subFuel.engine}>
                          {subFuel.engine}
                        </option>
                      ))}
                    </Form.Select>
              ...
              <Form.Group>
                <Form.Label className="fw-semibold mb-2 d-flex align-items-center" style={{ color: "#1e293b", fontSize: "0.9rem" }}>
                  <div className="w-2 h-2 bg-danger rounded-circle me-2" style={{ width: "6px", height: "6px" }}></div>
                  Firefighting Equipment CO2 Emissions *
                </Form.Label>
                <Form.Control
                  type="number"
                  placeholder="Enter Firefighting Equipment CO2 Emissions value (e.g., 1500)"
                  value={currentEntry.firefightingEquipmentCO2Emissions}
                  onChange={(e) => updateEntry("firefightingEquipmentCO2Emissions", e.target.value)}
                  disabled={!!fuelError || !!duplicateEntry}
                  className="border-2 py-2 ps-3 pe-4"
                  style={{
                    backgroundColor: (!!fuelError || !!duplicateEntry) ? "#fee2e2" : "white",
                    borderRadius: "8px",
                    fontSize: "0.9rem",
                    height: "44px",
                    cursor: (!!fuelError || !!duplicateEntry) ? "not-allowed" : "text",
        ...
        {/* Consumption */}
        {(currentEntry.category === 'stationary' || currentEntry.category === 'mobile') ? (
          <Row className="g-4 mb-4">
            <Col md={12}>
              <Form.Group>
                <Form.Label className="fw-semibold mb-2 d-flex align-items-center" style={{ color: "#1e293b", fontSize: "0.9rem" }}>
          ...
          <Row className="g-4 mb-4">
            <Col md={6}>
              <Form.Group>
                <Form.Label className="fw-semibold mb-2 d-flex align-items-center" style={{ color: "#1e293b", fontSize: "0.9rem" }}>
                  <div className="w-2 h-2 bg-danger rounded-circle me-2" style={{ width: "6px", height: "6px" }}></div>
                  AC/Refrigeration HFC Emissions *
                </Form.Label>
                <Form.Control
                  type="number"
                  placeholder="Enter AC/Refrigeration HFC Emissions value (e.g., 1500)"
                  value={currentEntry.acRefrigerationHfCEmissions}
                  onChange={(e) => updateEntry("acRefrigerationHfCEmissions", e.target.value)}
                  disabled={!!fuelError || !!duplicateEntry}
                  className="border-2 py-2 ps-3 pe-4"
                  style={{
                    backgroundColor: (!!fuelError || !!duplicateEntry) ? "#fee2e2" : "white",
            ...
            <Col md={6}>
              <Form.Group>
                <Form.Label className="fw-semibold mb-2 d-flex align-items-center" style={{ color: "#1e293b", fontSize: "0.9rem" }}>
                  <div className="w-2 h-2 bg-danger rounded-circle me-2" style={{ width: "6px", height: "6px" }}></div>
                  Substation SF6 Emissions *
                </Form.Label>
                <Form.Control
                  type="number"
                  placeholder="Enter Substation SF6 Emissions value (e.g., 1500)"
                  value={currentEntry.substationSf6Emissions}
                  onChange={(e) => updateEntry("substationSf6Emissions", e.target.value)}
                  disabled={!!fuelError || !!duplicateEntry}
                  className="border-2 py-2 ps-3 pe-4"
                  style={{
                    backgroundColor: (!!fuelError || !!duplicateEntry) ? "#fee2e2" : "white",
        ...
        {(
  (
    (stationaryFuel || selectedMobileFuel) && currentEntry.consumption
  ) ||
  (
    currentEntry.category === 'fugitive' &&
    currentEntry.firefightingEquipmentCO2Emissions &&
    currentEntry.acRefrigerationHfCEmissions &&
    currentEntry.substationSf6Emissions &&
    !duplicateEntry
  )
) && (
  <div 
  ...
  >
    ...
    <EmissionCalculationResults
      results={stationaryFuel || selectedMobileFuel || selectedNewFugitiveFuel}
      consumption={currentEntry.consumption}
      category={currentEntry.category}
      firefightingEquipmentCO2Emissions={currentEntry.firefightingEquipmentCO2Emissions}
      acRefrigerationHfCEmissions={currentEntry.acRefrigerationHfCEmissions}
      substationSf6Emissions={currentEntry.substationSf6Emissions}
          ...
          <Button
            ...
            onMouseOver={(e) => {
              if (isFormValid) {
                ...
                e.currentTarget.style.boxShadow = "0 12px 30px -5px rgba(102, 126, 234, 0.6), 0 8px 10px -2px rgba(0, 0, 0, 0.1)"
              }
            }}
            onMouseOut={(e) => {
              if (isFormValid) {
                ...
                e.currentTarget.style.transform = "translateY(0px)"
                e.currentTarget.style.boxShadow = "0 8px 25px -5px rgba(102, 126, 234, 0.5), 0 4px 6px -2px rgba(0, 0, 0, 0.05)"
              }
            }}
          >
