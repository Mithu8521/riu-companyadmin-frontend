...
export default class SectorQuestions extends Component {
  constructor(props) {
    super(props);
    this.state = {
      login: false,
    ...
    this.removeSectorQuestion = this.removeSectorQuestion.bind(this);
  }

  removeSectorQuestion(id) {
    ...
    Swal.fire({
      title: "Do you want to delete this Sector Question?",
      ...
      if (result.isConfirmed) {
        axios
          .delete(
            config.API_URL + `deleteSectorQuestion/${id}?current_role=${localStorage.getItem("role")}`, requestOptions,
            {
              id: id,
            }
          )
  ...
  componentDidMount() {
    ...
    fetch(
      config.API_URL +
      `getSectorQuestionByTabs?uri=/${tab_name}&industry_id=${id}&current_role=${localStorage.getItem("role")}`,
      requestOptions
    )
      .then((res) => res.json())
      .then(
                                        ...
                                        <tr key={key}>
                                          ...
                                          <td>
                                            <NavLink
                                              ...
                                              to={"/sector_questions/" + item.id + "/update_sector_question"}
                                            >
                                          ...
                                          <td className="red view_c" onClick={() => this.removeSectorQuestion(item.id)}>
