...
const TotalWasteGeneratedByTpe = ({
  title,
  ...
  useEffect(() => {
    if (productOptions) {
      if (productOptions.length > 0) {
        ...
        const productLabels = lastFiveProducts.map((product) => product.label);
    ...
    if (selectedProducts.length > 0) {
      ...
      dataSource.forEach((item) => {
        ...
        answers[0].forEach((answerArray, index) => {
          ...
          const value = answerArray; // First element corresponds to recoverySeries value
      ...
      {selectedProducts && selectedProducts.length > 0 && (
        <>
          <div className="renewable-bar-labels" style={{ marginTop: "5%" }}>
            ...
            >
              {totalSum % 1 !== 0
                ? formatValue(((totalSum / 5) * 3).toFixed(1))
  ...
  {selectedProducts
    ...
    .sort((a, b) => a.value - b.value) // Sort in ascending order (smaller values come first)
    .map(({ recoveryType, value, color }, index) => {
      ...
      const widthPercentage =
        totalSum > 0 ? (scaledValue / Math.log10(totalSum + 1)) * 100 : 0;
      ...
      return (
        <div
          ...
          key={index}
          title={`${displayRecoveryType}: ${value.toFixed(3)} `}
          style={{
