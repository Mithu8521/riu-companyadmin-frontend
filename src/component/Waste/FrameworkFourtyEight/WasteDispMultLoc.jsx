...
const VerticalBarComponent = ({ brief, type = null }) => {
  ...
  // Conversion function: converts Kg to kWh if needed
  const convertValue = (value) => {
    if (type === "ELE" || type === "REW") {
      // 1 Kg = 277.788 kWh, but requirement says multiply by 1000
      return (value * 2500) / 9;
    }
    return value;
  };

  // Function to clean product display names
  const cleanProductName = (name) => {
    if (!name) return "";

    // Remove "Total " from the beginning
    let cleanName = name.replace(/^Total\s+/i, "");

    // Remove everything after and including the asterisk (*)
    const asteriskIndex = cleanName.indexOf("*");
    if (asteriskIndex !== -1) {
      cleanName = cleanName.substring(0, asteriskIndex).trim();
    }

    return cleanName;
  };

  // Extract the list of product options based on the type, with cleaned names for display
  const productOptions = useMemo(() => {
    ...
    // Convert the list to options format for the dropdown with cleaned names for display
    return products.map((product) => ({
      label: cleanProductName(product), // Clean name for display
      value: product, // Original name for data lookup
    }));
  }, [type]);
  ...
  // Create a mapping of original keys to clean display names
  const displayNameMap = Object.keys(displayLegendSums).reduce((acc, key) => {
    acc[key] = cleanProductName(key);
    return acc;
  }, {});
  ...
  const actualTotalSum = Object.keys(displayLegendSums).reduce(
    (sum, key) => sum + displayLegendSums[key],
    0
  );
  ...
  const formatValue = (value) => {
    ...
    // Format Kg values with K suffix for thousands
    if (value >= 1000) {
      return `${(value / 1000).toFixed(1)}K`;
    }
    return value.toFixed(1);
      ...
      >
        ...
        >
          ...
          {Object.keys(displayLegendSums).map((key, index) => {
            ...
            // Use the clean display name for this product
            const displayName = displayNameMap[key];
            ...
            return (
              <div
                ...
                style={{
                  ...
                  alignItems: "center",
                  width: `${
                    100 / (Object.keys(displayLegendSums).length + 1)
                  }%`,
                  height: "100%",
                  position: "relative",
                }}
              >
                ...
                >
                  ...
                  <div
                    ...
                    title={`${displayName}: ${formatValue(
                      value
                    )} ${getUnit()} (${((value / actualTotalSum) * 100).toFixed(
                      1
                    )}% of total)`}
                  ></div>
                </div>

                {/* Label below the bar - using cleaned display name */}
                <div
                  style={{
                    position: "absolute",
