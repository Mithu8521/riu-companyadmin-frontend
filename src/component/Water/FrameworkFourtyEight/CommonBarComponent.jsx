...
const CommonBarComponent = ({ matchedDataWater, title, com }) => {
  const colors = [  
    "#E6594D", // Red
    "#3F822B", // Green
    "#1212F1", // Blue
    ...
    >
      ...
      <div style={{ 
        ...
        <div style={{ display: "flex", alignItems: "center" }}>
          <div style={{ 
            width: "15px", 
            borderTop: `2px dotted ${minTargetColor}`,
            marginRight: "5px" 
          }}></div>
        ...
        <div style={{ display: "flex", alignItems: "center" }}>
          <div style={{ 
            width: "15px", 
            borderTop: `2px dotted ${maxTargetColor}`,
            marginRight: "5px" 
          }}></div>
      ...
      {selectedProducts && selectedProducts.length > 0 && (
        <>
          ...
          >
            ...
            {/* Connected target lines - Using SVG with polyline */}
            <div style={{
              position: "absolute",
              left: "70px",
              width: "calc(100% - 70px)",
              ...
              <svg style={{
                ...
                {/* Create the connected min target line */}
                {selectedProducts.length > 0 && (
                  <polyline
                    points={selectedProducts
                      .map((product, index) => {
                        const minTarget = minTargets[product.value] || 0;
                        if (minTarget === 0) return null;
                        const segmentWidth = 100 / (selectedProducts.length + 1);
                        const xPos = segmentWidth * (index + 0.5);
                        const yPos = 100 - ((minTarget / maxValue) * 100);
                        return `${xPos},${yPos}`;
                      })
                      .filter(point => point !== null)
                      .join(' ')}
                    stroke={minTargetColor}
                    strokeWidth="2"
                    strokeDasharray="5,5"
                    fill="none"
                  />
                ...
                {/* Create the connected max target line */}
                {selectedProducts.length > 0 && (
                  <polyline
                    points={selectedProducts
                      .map((product, index) => {
                        ...
                        const segmentWidth = 100 / (selectedProducts.length + 1);
                        const xPos = segmentWidth * (index + 0.5);
                        const yPos = 100 - ((maxTarget / maxValue) * 100);
                        return `${xPos},${yPos}`;
                      })
                      .filter(point => point !== null)
                      .join(' ')}
                    stroke={maxTargetColor}
                    strokeWidth="2"
                    strokeDasharray="5,5"
                    fill="none"
                  />
                ...
                {/* Add min target dots on each point */}
                ...
                {/* Add max target dots on each point */}
                {selectedProducts.map((product, index) => {
                  const maxTarget = maxTargets[product.value] || 0;
                  if (maxTarget === 0) return null;
                  const segmentWidth = 100 / (selectedProducts.length + 1);
                ...
                {/* Connect to total min target */}
                {totalMinTarget > 0 && selectedProducts.some(p => minTargets[p.value] > 0) && (
                  <polyline
                    points={`
                      ${selectedProducts
                        .filter(p => minTargets[p.value] > 0)
                        .map((p, i) => {
                          const segmentWidth = 100 / (selectedProducts.length + 1);
                          const xPos = segmentWidth * (selectedProducts.findIndex(sp => sp.value === p.value) + 0.5);
                          const yPos = 100 - ((minTargets[p.value] / maxValue) * 100);
                          return `${xPos},${yPos}`;
                        })
                        .pop()
                      } 
                      ${100 - 100 / (selectedProducts.length + 1) / 2},${100 - (totalMinTarget / maxValue) * 100}
                    `}
                    stroke={minTargetColor}
                    strokeWidth="2"
                    strokeDasharray="5,5"
                ...
                {/* Connect to total max target */}
                {totalMaxTarget > 0 && selectedProducts.some(p => maxTargets[p.value] > 0) && (
                  <polyline
                    points={`
                      ${selectedProducts
                        .filter(p => maxTargets[p.value] > 0)
                        .map((p, i) => {
                          const segmentWidth = 100 / (selectedProducts.length + 1);
                          const xPos = segmentWidth * (selectedProducts.findIndex(sp => sp.value === p.value) + 0.5);
                          const yPos = 100 - ((maxTargets[p.value] / maxValue) * 100);
                          return `${xPos},${yPos}`;
                        })
                        .pop()
                      } 
                      ${100 - 100 / (selectedProducts.length + 1) / 2},${100 - (totalMaxTarget / maxValue) * 100}
                    `}
                    stroke={maxTargetColor}
                    strokeWidth="2"
                    strokeDasharray="5,5"
                ...
                {/* Total min target dot */}
                {totalMinTarget > 0 && (
                  <circle
                    cx={100 - 100 / (selectedProducts.length + 1) / 2}
                    cy={100 - (totalMinTarget / maxValue) * 100}
                    r="4"
                    fill={minTargetColor}
                  />
                )}

                {/* Total max target dot */}
                {totalMaxTarget > 0 && (
                  <circle
                    cx={100 - 100 / (selectedProducts.length + 1) / 2}
                    cy={100 - (totalMaxTarget / maxValue) * 100}
                    r="4"
                    fill={maxTargetColor}
                  />
                )}
...
Min Target: ${minTargets[product.value]?.toFixed(2) || 0} ${getUnit()}
Max Target: ${maxTargets[product.value]?.toFixed(2) || 0} ${getUnit()}`}
                      >
                      </div>
                    </div>
