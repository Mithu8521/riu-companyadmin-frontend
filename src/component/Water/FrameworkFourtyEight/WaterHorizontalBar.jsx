...
const WaterHorizontalBar = ({
  ...
  const [chartOptions, setChartOptions] = useState({
    ...
    dataLabels: {
      ...
      formatter: function (val) {
        return `${val.toFixed(0)}`; // Display value directly
      },
    ...
    legend: {
      ...
      formatter: function (seriesName, opts) {
        // Clean the series name for the legend
        const cleanedName = cleanProductName(seriesName);
        return `<div style="display: flex; align-items: center;">
                 <span style="color: #7b91b0;">${cleanedName}</span>
                </div>`;
      },
    },
  });
  ...
  // Create a mapping of original product names to cleaned names
  const [productNameMap, setProductNameMap] = useState({});

  // Constants for energy conversion
  const CONVERSION_FACTOR = 277.78; // Conversion factor from KL to kWh
  ...
  const energyUnit = isElectricityType ? "kWh" : "KL";
  ...
  useEffect(() => {
    // No log transform - just apply conversion if needed
    const transformValue = (value) => {
      // Apply conversion if needed based on type
    ...
    if (timePeriodValues?.length > 1 && locationOption?.length == 1) {
      if (brief && brief.location) {
        ...
        const series = Object.entries(productTotals).map(
          ([productKey, data]) => ({
            name: productKey,
            data: data.map(transformValue), // Apply conversion but no log transform
          })
        );
  ...
  }, [selectedProducts, brief, type, isElectricityType, energyUnit, CONVERSION_FACTOR]);
  ...
  const productOptions = useMemo(() => {
    ...
    if (brief?.location && Object.keys(brief.location).length > 0) {
      ...
      if (filteredProducts.length > 0) {
        // Create and update the product name mapping
        const newProductNameMap = {};
        filteredProducts.forEach(product => {
          newProductNameMap[product] = cleanProductName(product);
        });
        setProductNameMap(newProductNameMap);
        
        // Return options with cleaned labels
        return filteredProducts.map((product) => ({
          value: product, // Keep original value for data lookup
          label: cleanProductName(product), // Use cleaned name for display
        }));
      }
    }
  ...
  const CustomControl = (props) => {
    ...
    return (
      <components.Control {...props}>
        ...
        {/* Display only the first selected product with cleaned name */}
        {value && value.length > 0 && (
          <div
            style={{
              color: "#3f88a5",
          ...
          >
            {/* Use the cleaned label that's already in the value object */}
            {value[0].label}
