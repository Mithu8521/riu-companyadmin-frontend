...
const TeamWorkLoadChecker = (props) => {
  // Adding default value to handle undefined props
  ...
  const getBarColor = (type) => {
    ...
    const dataItem = orgMakerOtatusData.find((item) => item.name === type);
    ...
    switch (type) {
      case "pending":
        return "#0d6efd"; // Blue for Pending/Submitted
      case "approved":
        return "#2e7d32"; // Green for Approved
      case "revision":
        return "#f0a030"; // Orange for Requires Revision
      case "rejected":
        return "#f44336"; // Red for Rejected
      case "drafts":
        return "#808080"; // Gray for Drafts
      default:
        return "#0d6efd"; // Default light blue/gray
    }
  };
  ...
  // Get full label text for tooltips
  const getFullLabel = (type) => {
    switch (type) {
      case "pending":
  ...
  const handleMouseEnter = (event, type, value, percentage) => {
    ...
    const fullLabel = getFullLabel(type);
  ...
  const renderBarChart = () => {
    ...
    const orderedBarTypes = ["pending", "approved", "revision", "rejected"];
    ...
    const barTypes = orderedBarTypes.filter((type) =>
      ...
      orgMakerOtatusData.some((item) => item.name === type)
    );

    // If there's no data after filtering, use all ordered types as fallback
    if (barTypes.length === 0) {
      barTypes.push(...orderedBarTypes);
    }
      ...
      >
        ...
        <div
          style={{
            ...
            backgroundSize: `100% ${
              (chartHeight - topMargin - bottomMargin) / (yAxisTicks.length - 1)
            }px`,
          }}
        >
          {/* Grid lines */}
          {yAxisTicks.map((value) => (
            <div
              ...
              style={{
                ...
                left: "0",
                top: `${
                  chartHeight - bottomMargin - value * scaleFactor - topMargin
                }px`,
                width: "100%",
        ...
        >
          {barTypes.map((type, index) => {
            // Find actual data from the provided array
            const dataItem = orgMakerOtatusData.find(
              (item) => item.name === type
            );

            // Use data values or defaults
            ...
            const leftPosition =
              gapBetweenBars * (index + 1) + barWidth * index;
            ...
            return (
              <div
                ...
                onMouseEnter={(e) =>
                  handleMouseEnter(e, type, value, percentage)
                }
                onMouseLeave={handleMouseLeave}
                onClick={() => handleBarClick(type)} // Add click handler for navigation with type parameter
              >
