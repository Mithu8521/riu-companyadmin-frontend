...
const DiversityBarComponent = ({
  ...
  console.log(dataOne, "dataOnedataOnedataOne");
  ...
  const categoryValues = uniqueCategories.map((category, categoryIndex) => {
    ...
    // Get min and max targets for this category
    const minTarget = dataOne.reduce((min, item) => {
      if (item.minTarget && item.minTarget[0] && item.minTarget[0][categoryIndex] !== undefined) {
        const minValue = Number(item.minTarget[0][categoryIndex]);
    ...
    const maxTarget = dataOne.reduce((max, item) => {
      if (item.maxTarget && item.maxTarget[0] && item.maxTarget[0][categoryIndex] !== undefined) {
        const maxValue = Number(item.maxTarget[0][categoryIndex]);
  ...
  const targetLineWidth = 70; // Set target line width to 70px as requested
  ...
  // For vertical scaling - use max target value as max for proper scaling if it's higher than total value
            ...
            <div style={{ 
              ...
              <div style={{
                ...
                {Array.from({ length: yAxisSteps + 1 }).map((_, index) => {
                  const value = ((yAxisSteps - index) * increment).toFixed(1);
