...
const AuditorWorkload = ({ user, teamWorkloadData }) => {
  ...
  const prepareChartData = () => {
    ...
    // Map your data to match the 4 categories in the image
    // Reviewed (Green) - maps to Approved
    if (activeLegend.includes("Reviewed")) {
      const reviewedData = currentPageData.map(item => {
        if (isUser) {
      ...
      maxValue = Math.max(maxValue, ...reviewedData);
    ...
    // Pending (Yellow/Orange) - maps to Not Responded
    if (activeLegend.includes("Pending")) {
      const pendingData = currentPageData.map(item => {
        if (isUser) {
          return Number(item?.answerNotResponded || 0);
        } else {
          return Number(item?.notResponded || 0);
        }
      });

      maxValue = Math.max(maxValue, ...pendingData);
    ...
    // Rejected (Red)
    if (activeLegend.includes("Rejected")) {
      const rejectedData = currentPageData.map(item => {
        if (isUser) {
      ...
      maxValue = Math.max(maxValue, ...rejectedData);
    ...
    // // Revision Required (Teal/Blue) - we'll use a portion of pending for this
    // if (activeLegend.includes("Revision Required")) {
    //   const revisionData = currentPageData.map(item => {
    //     // Since your data doesn't have revision required, we'll simulate it
    //     // You can map this to actual revision data when available
    //     const baseValue = isUser ? 
    //       Number(item?.answerNotResponded || 0) : 
    //       Number(item?.notResponded || 0);
    //     return Math.floor(baseValue * 0.3); // 30% of pending as revision required
    //   });
    ...
    return (
      <div style={{
        ...
        <p style={{
          ...
          Assign tasks to team members to view performance analytics.
        </p>
      </div>
    );
  }
    ...
    <div style={{
      ...
      <div style={{
        ...
        <h2 style={{
          ...
          {user === "user" ? "Data Processing Dashboard" : "Auditor Performance Analytics"}
        </h2>
      </div>
      ...
      <div style={{
        ...
        <div
          style={{
            ...
            alignItems: "center",
            cursor: "pointer",
            opacity: activeLegend.includes("Reviewed") ? 1 : 0.5
          }}
          onClick={() => handleLegendClick("Reviewed")}
        >
          <div style={{
            width: "16px",
            height: "16px",
          ...
          <span style={{ fontSize: "14px", color: "#374151", fontFamily: "Arial, sans-serif" }}>Reviewed</span>
        </div>
        ...
        <div
          style={{
            ...
            alignItems: "center",
            cursor: "pointer",
            opacity: activeLegend.includes("Pending") ? 1 : 0.5
          }}
        ...
        >
          ...
          <span style={{ fontSize: "14px", color: "#374151", fontFamily: "Arial, sans-serif" }}>Pending</span>
        </div>
        ...
        <div
          style={{
            ...
            alignItems: "center",
            cursor: "pointer",
            opacity: activeLegend.includes("Rejected") ? 1 : 0.5
          }}
        ...
        >
          ...
          <span style={{ fontSize: "14px", color: "#374151", fontFamily: "Arial, sans-serif" }}>Rejected</span>
        </div>
...
export default AuditorWorkload;