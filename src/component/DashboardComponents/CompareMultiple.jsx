...
const CompareMultiple = ({ locationOption, timePeriods, graphData, type, productTypeOptions = null }) => {
  const colorPalette =
    type === "BIO"
      ? [
        "#FF0000", // Red
  ...
  const fuelTypeOptions = useMemo(() => {
    ...
    graphData.forEach((item) => {
      const fuel =
        typeof item?.fuelType === "string" ? item.fuelType.trim() : "";
      const title = typeof item?.title === "string" ? item.title.trim() : "";
      const value = fuel || title;

      if (
        value &&
        !allFuelTypes.includes(value) &&
        (productTypeOptions?.includes(value) || typeCate[type]?.includes(value))
      ) {
        allFuelTypes.push(value);
      }
    });
  ...
  }, [graphData, typeCate, type, productTypeOptions]);
  ...
  useEffect(() => {
    ...
    const maxDataValue = calculateMaxValue(newChartSeries);
    ...
    for (const location of locationKey) {
      ...
      for (const fuelType of allFuelTypes) {
        for (let k = 0; k < timeKey.length; k++) {
          ...
          // FIXED: Corrected filterData logic for time data
          const filterData = filteredData.find((item) => {
            return (item?.fuelType === fuelType || item?.title === fuelType) &&
              ((type === "MANUFACTURINGINTENSITY" || type === "HOSPITALINTENSITY")
          ...
          if (data[fuelType]) {
            if (type === "MANUFACTURINGINTENSITY" || type === "HOSPITALINTENSITY") {
              data[fuelType][k] = Number(filterData?.intensity) || 0;
            } else {
    ...
    for (const time of timeKey) {
      ...
      for (const fuelType of allFuelTypes) {
        for (let k = 0; k < locationKey.length; k++) {
          ...
          // FIXED: Corrected filterData logic for location data
          const filterData = filteredData.find((item) => {
            return (item?.fuelType === fuelType || item?.title === fuelType) &&
              ((type === "MANUFACTURINGINTENSITY" || type === "HOSPITALINTENSITY")
                ? item?.fromDate === formDate
  ...
  }, [locationOption, timePeriods, graphData, typeCate, type, productTypeOptions, view]);
