...
const TotalEnergySingLocMultTime = ({
  ...
  useEffect(() => {
    if (locationOption.length > 1 && timePeriodValues.length === 1) {
      const categories = locationOption.map((loc) =>
      ...
      const series = categories.map((category) => {
        ...
        const renewableTotal = renewableEnergy
          ...
          .reduce((sum, item) => {
            const value = item.energyAndEmission[1];
      ...
      setChartSeries([{ name: "Total Emission", data: series }]);
      ...
      const categoriesTwo = Object.keys(timePeriods).map((key) =>
      ...
      const series = categories.map((key) => {
        ...
        .reduce((sum, item) => {
          // Remove the first row and sum up the second element of each subsequent array in energyAndEmission
          const total = item.energyAndEmission.slice(1).reduce((innerSum, arr) => { // Skip the first element
            const numValue = Number(arr[1]); // Access and convert the second element to a number
            return innerSum + (isNaN(numValue) || arr[1] === "" ? 0 : numValue);
          }, 0);
          return sum + total; // Accumulate total energy for renewable energy
        }, 0);
        ...
        .filter((item) => item.formDate === timePeriods[key]) // Match formDate with timePeriods[key]
        .reduce((sum, item) => {
          // Only consider the first row of energyAndEmission
          const firstRow = item.energyAndEmission[0]; // Get the first row
          const numValue = Number(firstRow[1]); // Access the first element of the first row
          return sum + (isNaN(numValue) || firstRow[1] === "" ? 0 : numValue); // Add the value if it's valid
        }, 0);
        ...
        const renewableTotal = renewableEnergy
          ...
          .reduce((sum, item) => {
            ...
            const total = item.energyAndEmission.reduce((innerSum, arr) => {
              const numValue = Number(arr[1]); // Convert to number
              return (
                innerSum + (isNaN(numValue) || arr[1] === "" ? 0 : numValue)
              );
            }, 0);
  ...
  return (
    <div className="container" style={{ height: "100%" }}>
      <div style={{ height: "10%", fontSize: "20px", fontWeight: 600, fontColor: "#011627" }}>Total Emission</div>
      <div style={{ height: "90%" }}>
