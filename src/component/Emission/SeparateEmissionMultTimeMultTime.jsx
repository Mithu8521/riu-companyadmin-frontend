...
const SeparateEmissionMultTimeMultTime = ({
  ...
  const [chartOptions, setChartOptions] = useState({
    ...
    stroke: {
      show: true,
      width: [2, 4],
      colors: ["#83bbd5", "#ff0000"], // Blue for Actual, Red for Trigger
  ...
  const getTotalEnergyForPeriod = (fromDate) => {
    return graphData
      ...
      .map((data) =>
        data.energyAndEmission.reduce((sum, energyPair) => {
          const value = Number(energyPair[1]);
          return sum + (isNaN(value) || value === "" ? 0 : value);
        }, 0)
      )
  ...
  const getTotalTargetEnergyForPeriod = (fromDate) => {
    return graphData
      .filter((data) => data.formDate === fromDate)
      .map((data) =>
        (data.targetData || []).flat().reduce((sum, value) => {
  ...
  return (
    <div className="container" style={{ height: "100%" }}>
      ...
      >
        Period-wise Emission Consumption
      </div>
      <div style={{ height: "90%" }}>
...
export default SeparateEmissionMultTimeMultTime;
