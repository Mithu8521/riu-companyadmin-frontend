...
const SafetyMultiBar = ({ type, number, timePeriods, matchedDataWater }) => {
  // Reference to the container div to get its dimensions
  ...
  useEffect(() => {
    ...
    // Create a ResizeObserver to watch the container size changes
  ...
  const isVerySmallScreen = windowDimensions.width < 480;
  ...
  // Colors for each category - matching the example image
  const colors = [
    "#3366CC", // Rich Blue
    "#DD4477", // Rust Red
    "#FF9900", // Amber Orange
  ...
  const calculateResponsiveValues = () => {
    ...
    const titleHeight = isSmallScreen ? 20 : 30;
    const xAxisHeight = isSmallScreen ? 25 : 30;
    
    // Modified legend height calculation for better spacing
    const legendRowCount = Math.ceil(uniqueCategories.length / 
      (isVerySmallScreen ? 2 : (isSmallScreen ? 3 : 4)));
    const legendHeight = uniqueCategories.length > 0 
      ? legendRowCount * (isSmallScreen ? 20 : 22)
      : 0;
    
    // Calculate available height for the chart
    const availableHeight = Math.max(
      150, // Minimum height
      containerDimensions.height - padding * 2 - titleHeight - xAxisHeight - legendHeight
    );
    
    return {
      chartHeight: availableHeight,
      barWidth: Math.max(
        isVerySmallScreen ? 20 : 30,
        Math.min(60, (containerDimensions.width - padding * 2) / (seriesData.length * 2))
      ),
      titleFontSize: isSmallScreen ? "16px" : "18px",
      labelFontSize: isVerySmallScreen ? "10px" : (isSmallScreen ? "11px" : "12px"),
      valueFontSize: isVerySmallScreen ? "10px" : (isSmallScreen ? "11px" : "12px"),
      // Modified legend item width - more compact
      legendItemWidth: isVerySmallScreen ? "50%" : (isSmallScreen ? "33%" : "25%"),
    };
  };
  ...
  return (
    ...
    >
      ...
      <div style={{ 
        ...
        display: "flex",
        flexDirection: "column",
        minHeight: "150px" // Ensures a minimum height for the chart area
      }}>
        {/* Chart Container */}
        <div style={{ 
          display: "flex",
          flex: 1,
          position: "relative",
          marginBottom: isSmallScreen ? "25px" : "30px" // Space for x-axis labels
        }}>
          {/* Y-Axis Labels */}
          <div style={{ 
              ...
              {seriesData.map((series, seriesIndex) => {
                ...
                return (
                  ...
                  >
                    ...
                    >
                      {series.name.includes("-") ? series.name : series.name.charAt(0).toUpperCase() + series.name.slice(1)}
                    </div>
                  </div>
                );
              })}
        ...
        {/* Improved Legend - centered and more compact */}
        <div style={{
          display: "flex",
          flexWrap: "wrap",
          justifyContent: "center", // Center the legend items
          marginTop: isSmallScreen ? "8px" : "10px",
          marginBottom: "0", // Remove bottom margin to minimize empty space
        }}>
            ...
            >
              <div
                style={{
                  ...
                  marginRight: isSmallScreen ? "4px" : "6px",
                  flexShrink: 0
                }}
              />
              <div style={{ 
